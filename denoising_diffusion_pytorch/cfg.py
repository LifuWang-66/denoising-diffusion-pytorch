...
def train():
    ...
    if train_load_weight:
        ckpt = torch.load(os.path.join(
                    "../checkpoints/cfg", train_load_weight), map_location=device)
    ...
    for e in range(epoch):
        ...
        torch.save(net_model.state_dict(), os.path.join(
            "../checkpoints/cfg", 'ckpt_' + str(e) + "_.pt"))
...
def sample():
    ...
    ckpt = torch.load(os.path.join(
                "../checkpoints/cfg", test_load_weight), map_location=device)
    net_model.load_state_dict(ckpt)
    print("model load weight done.")
    net_model.eval()
    diffusion = GaussianDiffusion(
    ...
    for i in range(w):
        ...
        student_sampledImgs = diffusion.sample(
            classes = labels,
            cond_scale = i            # condition scaling, anything greater than 1 strengthens the classifier free guidance. reportedly 3-8 is good empirically
        )
        sample_list.append(student_sampledImgs)
    sample_list = torch.cat(sample_list, dim=0)       
    save_image(sample_list, os.path.join(
                "../images",  "cfgImages.png"), nrow=10)
# example
